...
Зафиксируй scope сравнения: для branch/PR — подходящую базу, для незакоммиченных изменений — также staged/unstaged diff и относящиеся к задаче новые файлы. Сравнение только с `HEAD` не охватывает все эти случаи. Не меняй checkout для чтения diff.

Проверь два аспекта:

- **Требования:** найди исходный запрос, acceptance criteria или указанную спецификацию. Проверь пропущенное/частично реализованное поведение, неверные условия и добавленное вне scope. Для замечания назови конкретное требование и покажи расхождение в коде; не выводи требования из самой реализации.
- **Корректность и правила проекта:** проверь риски ниже и применимые соглашения. Ссылайся на правило, если называешь его нарушением; личное предпочтение не является стандартом проекта.

Если спецификации нет, используй явный запрос и известные контракты, отметь ограничение и продолжи проверку корректности. Уточняй только неоднозначность, которая меняет вывод. Не требуй issue tracker или отдельный spec-файл ради ревью.
...
Мысленно убери спорную обёртку: исчезнет ли лишняя сложность или callers придётся повторять её правила и обработку ошибок? Во втором случае она может быть полезной границей. Учитывай неявное поведение аннотаций, security и transactions; отсутствие логики в теле метода ещё не доказывает бесполезность.
...
Различай расхождения с требованиями и риски корректности/сопровождения; один дефект не дублируй. Выводи существенные findings по приоритету, чтобы stylistic concerns не скрывали неправильное поведение. Полноту реализации без доступных требований не объявляй подтверждённой.
